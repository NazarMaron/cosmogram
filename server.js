...
db.serialize(() => {
    ...
    db.run("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, sender TEXT, receiver TEXT, text TEXT)");
    db.run("CREATE TABLE IF NOT EXISTS friends (user TEXT, friend TEXT, UNIQUE(user, friend))");
});
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        db.run("INSERT INTO users (username, password, avatar) VALUES (?, ?, ?)", [username, hashedPassword, defaultAvatar], (err) => {
            if (err) return res.status(400).json({ error: "Цей нікнейм вже зайнятий!" });
...
app.post('/upload-avatar', upload.single('avatarImage'), (req, res) => {
    ...
    if (!req.file || !username) return res.status(400).json({ error: "Файл не отримано" });

    // Шлях до нової картинки
    const avatarUrl = '/uploads/' + req.file.filename;

    // Оновлюємо базу даних
    db.run("UPDATE users SET avatar = ? WHERE username = ?", [avatarUrl, username], (err) => {
...
io.on('connection', (socket) => {
    ...
    socket.on('addFriend', (friendUsername) => {
        ...
        db.get("SELECT username FROM users WHERE LOWER(username) = LOWER(?)", [friendUsername], (err, row) => {
            if (!row) return socket.emit('friendError', `Акаунт "${friendUsername}" не знайдено!`);
            const exactFriendName = row.username;
            if (exactFriendName === me) return socket.emit('friendError', "Не можна додати себе!");

            db.run("INSERT OR IGNORE INTO friends (user, friend) VALUES (?, ?), (?, ?)", 
            [me, exactFriendName, exactFriendName, me], (err) => {
                if (!err) {
                    sendFriendList(socket, me);
                    if (userSockets[exactFriendName]) {
                        sendFriendList(io.sockets.sockets.get(userSockets[exactFriendName]), exactFriendName);
                    }
                }
            });
        });
    ...
    socket.on('getHistory', (withUser) => {
        const me = socket.username;
        db.all("SELECT * FROM messages WHERE (sender = ? AND receiver = ?) OR (sender = ? AND receiver = ?) ORDER BY id ASC", 
        [me, withUser, withUser, me], (err, rows) => {
            if (err) return;
    ...
    socket.on('privateMessage', ({ receiver, text }) => {
        ...
        db.run("INSERT INTO messages (sender, receiver, text) VALUES (?, ?, ?)", [sender, receiver, text], function(err) {
            ...
            if (userSockets[receiver]) io.to(userSockets[receiver]).emit('newPrivateMessage', messageData);
...
server.listen(3000, () => console.log('Сервер працює на http://localhost:3000'));